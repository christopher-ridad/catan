...
| `getWinner(Game game, Board board, SpecialCardTracker tracker, Player currentPlayer)` | `Optional<Player>` | Returns the player with ≥ 10 VP if one exists, or `Optional.empty()` otherwise; if multiple players somehow reach 10 VP simultaneously, returns the one whose turn it currently is                      |
| `hasWinner(Game game, Board board, SpecialCardTracker tracker, Player currentPlayer)` | `boolean`          | Returns true if any player has ≥ 10 VP; convenience wrapper around `getWinner()`                                                                                                                        |
